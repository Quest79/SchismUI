...
local function GetTemplate(t)
	if t == "Tukui" then
		borderr, borderg, borderb, borderba = .26, .26, .26, 1
		backdropr, backdropg, backdropb, backdropa = .08, .08, .08, 1
	...
	elseif t == "Transparent2" then
		borderr, borderg, borderb = .16, .16, .16, 1
		backdropr, backdropg, backdropb, backdropa = unpack(C["media"].backdropfadecolor)
		
	elseif t == "Quest" then
		borderr, borderg, borderb, borderba = .25, .25, .25, 1
		backdropr, backdropg, backdropb, backdropa = .8, .8, .8, 1

	else
		--borderr, borderg, borderb = unpack(C["media"].bordercolor)
		--backdropr, backdropg, backdropb = unpack(C["media"].backdropcolor)
		borderr, borderg, borderb, borderba = .22, .22, .22, 1
		backdropr, backdropg, backdropb, backdropa = .0, .0, .0, 1
	end
end

--unpack(C["media"].shadowintensity)
---------------------------------------------------
-- END OF TEMPLATES								 --
---------------------------------------------------
...
--power:AllPoints(self.Health, OFFSET, -OFFSET)

local function AllPoints(eobj, earg1, earg2, earg3)
	eobj:Point("TOPLEFT", earg1, "TOPLEFT", earg2, earg3)
	eobj:Point("BOTTOMRIGHT", earg1, "BOTTOMRIGHT", earg2, earg3)
end
...
local function SetTemplate(f, t, texture)
    ...
    if t == "NoBorder" then
        ...
        f:SetBackdropColor(borderr, borderg, borderb, backdropa)
    else
        f:SetBackdrop({
          bgFile = C["media"].normTex2,
...
local function CreateShadow(f, t)
	...
	shadow:SetBackdropBorderColor(borderr, borderg, borderb, unpack(C["media"].shadowintensity))
...
local function StyleButton(b, c) 
	...
	if cooldown then
		...
		cooldown:SetBackdropBorderColor(1, 0, 0) -- Sets a shading color for the frame's border graphic
		cooldown:SetBackdropColor(1, 1, 0) -- Sets a shading color for the frame's background graphic
