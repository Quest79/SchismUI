...
function E.Install()
	...
	SetCVar("gxTextureCacheSize", 384)
	
	SetCVar("gxRefresh", 60)
	...
	if (C.chat.enable == true) and (not IsAddOnLoaded("Prat") or not IsAddOnLoaded("Chatter")) then					
		...
		for i = 1, NUM_CHAT_WINDOWS do
			...
			_G["ChatFrame"..i]:SetSize(E.Scale(C["chat"].chatwidth - 5 - 200), E.Scale(C["chat"].chatheight))
			
			-- this is the default width and height of Elvui chats.
			SetChatWindowSavedDimensions(chatFrameId, E.Scale(C["chat"].chatwidth + -4 - 200), E.Scale(C["chat"].chatheight))
