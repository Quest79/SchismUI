...
-- create the bar
local ElvuiThreatBar = CreateFrame("StatusBar", "ElvuiThreatBar", LocationBar)
...
ElvuiThreatBar:FontString(nil, C.media.pixel, 9, 'THINOUTLINE, MONOCHROME')
ElvuiThreatBar.text:SetPoint("CENTER")
ElvuiThreatBar.text:SetShadowOffset(0, 0)
...
local function OnUpdate(self, event, unit)
	if UnitAffectingCombat(self.unit) then
		...
		--print(E.ValColor)
		--E.RGBToHex(unpack(self.Colors[1]))
		
		--return string.format("|cff%02x%02x%02x", r*255, g*255, b*255)
		
		
		if E.Role ~= "Tank" or pet == 1 then
		...
		if threatval > 0 then
			CombatFX2(ElvuiThreatBar)
		else
			CombatFX3(ElvuiThreatBar)
		end		
	end
end
