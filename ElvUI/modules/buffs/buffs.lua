...
ConsolidatedBuffs:Point("LEFT", Minimap, "LEFT", 0, 0)
...
local aurapos = "RIGHT"
local buffsize = E.Scale(32)
local debuffsize = E.Scale(42)

--Holder frame for mover
local holder = CreateFrame("Frame", "AurasHolder", UIParent)
holder:Point("TOPRIGHT", Minimap, "TOPLEFT", -8, 2)
holder:SetWidth(buffsize*rowbuffs+(-(rowbuffs-1)*btnspace))
holder:SetHeight(ElvuiMinimap:GetHeight() + 22)
...
for i = 1, 3 do
	...
	f:CreatePanel("Default", buffsize, buffsize, "CENTER", _G["TempEnchant"..i], "CENTER", 0, 0)	
	...
	_G["TempEnchant"..i.."Duration"]:Point("BOTTOM", 0, 1)
...
local function StyleBuffs(buttonName, index, debuff)
	...
	if icon and not _G[buttonName..index.."Panel"] then
		...
		duration:Point("BOTTOM", 0, 0)
		duration:SetFont(C["media"].pixel, 9, "THINOUTLINE, MONOCHROME")
		duration:SetTextColor(1, 1, 0, 1)
		...
		if debuff then
			panel:CreatePanel("Default", debuffsize, debuffsize, "CENTER", buff, "CENTER", 0, 0)
		else
			panel:CreatePanel("Default", buffsize, buffsize, "CENTER", buff, "CENTER", 0, 0)		
		end
		...
		if ( buff.consolidated ) then
			...
			if aurapos == "RIGHT" then
				if ( (index > 1) and (mod(index, rowbuffs) == 1) ) then
					if ( index == rowbuffs*2+1 ) then
						buff:SetPoint("TOPRIGHT", AurasHolder, "TOPRIGHT", 0, -((buffsize+6)*2))
					elseif ( index == rowbuffs+1 ) then
						buff:SetPoint("TOPRIGHT", AurasHolder, "TOPRIGHT", 0, -(buffsize+6))
					else
