...
if ( E and C["media"] and C["media"]["pixel"] ) then
	-- Sets font for all texts
	MASTER_FONT = { C["media"]["pixel"], 9, "OUTLINE, MONOCHROME" };

	-- Sets font for stack count
	STACKS_FONT = { C["media"]["pixel"], 9, "OUTLINE, MONOCHROME" };
else
	-- Sets font for all texts
		...
		do
			...
			CreateAuraBar = function( parent )
				...
				if ( bit.band( ICON_POSITION, 4 ) == 0 ) then		
					...
					stacks:SetPoint( "CENTER", icon, "CENTER", 4, E.mult*2);
				...
				name:SetPoint( "BOTTOMRIGHT", bar, "BOTTOMRIGHT", -45, 1 );
				result.name = name;
				...
				time:SetPoint( "BOTTOMRIGHT", bar, "BOTTOMRIGHT", -TEXT_MARGIN, 1 );
				result.time = time;
