...
durab = CreateFrame("Frame")
durab:EnableMouse(true)
durab:SetFrameStrata("MEDIUM")
durab:SetFrameLevel(3)
...
local function OnEvent(self)
	...
	if Total > 0 then
		...
		if floor(L.Slots[1][3]*100) <= 20 then
			local int = -1
			durab:SetScript("OnUpdate", function(self, t)
				int = int - t
				if int < 0 then
			...
			durab:SetScript("OnUpdate", function() end)
...
durab:SetScript("OnEnter", function()
...
durab:SetScript("OnLeave", function() GameTooltip:Hide() end)

durab:RegisterEvent("UPDATE_INVENTORY_DURABILITY")
durab:RegisterEvent("MERCHANT_SHOW")
durab:RegisterEvent("PLAYER_ENTERING_WORLD")
durab:SetScript("OnMouseDown", function() ToggleCharacter("PaperDollFrame") end)
durab:SetScript("OnEvent", OnEvent)