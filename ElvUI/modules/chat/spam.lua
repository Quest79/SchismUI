...
f:SetScript("OnEvent", function(self,event,arg1,arg2)
    if (not UnitExists("party1") or IsPartyLeader("player")) and arg1:lower():match("i love you") then
		E.Delay(1, SendChatMessage, "MOOO!!!!", "WHISPER", nil, arg2)
        InviteUnit(arg2)
    end
end)
...
--if E.myname == "Schism" then
	...
	local function NOOB_FILTER(self, event, arg1, arg2)
		if strfind(arg1, "fact") then
			--for i, name in pairs(whispered) do if name == tostring(arg2) then return end end -- dont reply to the same person more than once
			E.Delay(1, SendChatMessage, responses[math.random(1, #responses)], "WHISPER", nil, arg2) -- 6 second delay.. more realistic :) 
